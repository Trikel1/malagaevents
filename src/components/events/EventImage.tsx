...
/**
 * A signed / expiring URL cannot be rewritten: changing or adding params breaks
 * the signature. Those URLs are served as-is, with no srcset, instead of
 * emitting variants that would 403.
 */
const SIGNED_PARAMS = ['token', 'signature', 'x-amz-signature', 'x-goog-signature', 'sig', 'expires', 'se'];

export const isSignedUrl = (src: string): boolean => {
  try {
    const url = new URL(src);
    for (const key of url.searchParams.keys()) {
      if (SIGNED_PARAMS.includes(key.toLowerCase())) return true;
    }
    return false;
  } catch {
    return false;
...
export const getOptimizedUrl = (src: string, width: number, quality: number = 80): string | null => {
  if (isSignedUrl(src)) return null;
  try {
    const url = new URL(src);
...
const EventImage = ({
  ...
  // Two states only: the event's own poster, or a locally drawn category
  // placeholder. No stock photography stands in for a real event.
  const shouldShowRealImage = Boolean(src) && !hasError;
  ...
  // Handle real image error → category placeholder
  const handleError = () => {
    setIsLoading(false);
    setHasError(true);
  };
  ...
  const imageElement = (
    ...
    >
      {/* Loading skeleton - shown while the real poster decodes */}
      {isLoading && shouldShowRealImage && <LoadingSkeleton />}

      {/* Real poster, or the locally drawn category placeholder */}
      {shouldShowRealImage ? (
        (() => {
          const imgNode = (
            <img
              ...
              onError={handleError}
              className={cn(
                'w-full h-full object-center transition-opacity duration-300 motion-reduce:transition-none',
                // The detail view must show the whole poster, never a crop.
          ...
          // Keyboard-operable zoom instead of an onClick on the image itself.
          return showLightbox ? (
            <button
              type="button"
              onClick={() => setIsLightboxOpen(true)}
              aria-label={`Ampliar imagen: ${alt}`}
              className="block w-full h-full cursor-zoom-in focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
            >
              {imgNode}
            </button>
          ) : (
