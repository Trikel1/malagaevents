// Sala Trinchera (Málaga) — dry-run only.
// Source: https://salatrinchera.com/
...
const BASE = "https://salatrinchera.com";
const LISTING_URLS = [
  "https://salatrinchera.com/agenda/",
  "https://salatrinchera.com/conciertos/",
  "https://salatrinchera.com/",
];

function inferCategory(title: string): string {
  const t = stripAccents(title.toLowerCase());
  if (/\b(concierto|musica|jazz|flamenco|dj|rock|indie|pop|electronica|festival|gira|tour)\b/.test(t)) return "music";
  if (/\b(monologo|teatro|comedia|stand.?up)\b/.test(t)) return "theater";
  if (/\b(taller|clase|workshop)\b/.test(t)) return "other";
  if (/\b(infantil|familiar|ninos)\b/.test(t)) return "kids";
  return "music";
}

export const salaTrincheraAdapter: SourceAdapter = {
  key: "sala-trinchera",
  name: "Sala Trinchera",
  fetchEvents: async (ctx) => {
    ...
    for (const url of LISTING_URLS) {
      try {
        ...
        ctx.logger.warn("sala-trinchera: fetch failed, trying next", {
          url,
    ...
    if (!markdown) {
      ctx.logger.error("sala-trinchera: no listing accessible", { tried: LISTING_URLS });
      return [];
    }

    const candidates = extractAgendaCandidates(markdown, BASE, {
      hrefFilter: /(agenda|concierto|evento|entrada|producto|shop)/i,
      excludeFilter:
    ...
    if (candidates.length === 0) {
      ctx.logger.warn("sala-trinchera: no_current_events", {
        url: usedUrl,
    ...
    ctx.logger.info("sala-trinchera: normalised", {
      listing: usedUrl,
